...
3. AntiDDoS AMR 配置（ChallengeAllDuringEvent、豁免正则、SEO 影响）
4. Challenge 动作适用性（POST/API/原生 App 限制）
5. Bot Control 配置（Allow 覆盖风险、verified vs unverified bot）
6. Token 和标签依赖（标签来源正确性）
7. 速率规则（激活延迟、阈值合理性）
8. IP 信誉和匿名 IP 规则
9. 跨规则依赖分析（修复顺序、标签链断裂）
10. 双 AMR 实例模式
11. Landing Page 和 Cookie 逻辑
12. 规则优先级排序（标签生产者在消费者之前）
13. 缺失的基线防护（CRS、KnownBadInputs）
14. WCU 容量感知
15. Token Domain 配置
16. 托管规则组版本
17. 日志和监控
